...
class Fish
{
  ...
  void draw()
  {
    ...
    for (int i = 0; i <= 180; i+=20) {
      ...
      float angle = sin(radians(i+s+frameCount*15)) * 60;
      vertex(x-angle, i*2);
      vertex(x-angle, i*2);
  ...
  void mouseAttract() {
    ...
    {
      magnetism = 3.5f;      // make particles be attracted to the mouse
    } else
    {
      magnetism = 01.0f;    // make particles be repulsed by the mouse
    }
